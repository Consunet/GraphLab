...
var graph = function(parameter_1, parameter_2) {
  ...
  this.graphFunction = function(ind, width) {
    ...
    if (this.centre) {
      ...
      while (z > - width - 1) {
        ...
        this.ctx.beginPath();
        while (x <= width) {
          this.lineto(x, (f(x * gww - this.x_shift, ZZ)
            + this.y_shift) * wgh, z);
          x += 1;
        }
        this.ctx.stroke();
    ...
    if (this.centre) {
      ...
      while (x > - width - 1) {
        ...
        while (z <= width) {
          ZZ = z * gww;
          this.lineto(x, (f(XX, z * gzw - this.z_shift)
            + this.y_shift) * wgh, z);
          z += 1;
        }
        this.ctx.stroke();
  ...
  this.insertFunction = function(f, name, colour) {
    ...
    this.function_colours [name] = (
      colour === undefined ? this.colours.shift() : colour
    );
  };
  this.insertSwitchPanel = function() {
    ...
    function_names.forEach(function(ind) {
      ...
      checkbox.onchange = function() {
        grapher.function_switches [this.id] = this.checked; grapher.reGraph();
  ...
  this.canvas.onmousemove = function(e) {
    if (e.buttons === 1) {
      grapher.roll(e.movementY / 100, e.movementX / 100); return;
    }
    if (e.buttons === 2) {grapher.move(e.movementX, e.movementY); return;}
  };
  ...
  this.createParameterBlock = function(div) {
    this.parameter_block = (
      div === null ? null : document.getElementById(div)
    );
  ...
  this.createParameter = function(parameter, from, to, step, init) {
    ...
    this.parameter_block
      .appendChild(document.createTextNode(" " + parameter + " = "));
    this.parameter_block.appendChild(span);
    this.parameter_block.appendChild(document.createElement("br"));
  };
  this.animate = function (delay, f) {
    if (f === undefined) {
      setInterval(function() {grapher.reGraph();}, delay);
    } else {
      setInterval (function() {f(); grapher.reGraph();}, delay);
    }
  };
  if (this.setup !== undefined) {
    Object.keys(this.setup).forEach(function(key) {
      ...
      switch (key) {
        case "roll": grapher.alpha = value; grapher.reAlpha(value); break;
        case "yaw": grapher.beta = value; grapher.reBeta(value); break;
        case "range_y":
